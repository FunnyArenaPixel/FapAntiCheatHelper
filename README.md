...
**对服务器性能影响极小。** 所有服务端处理为纯内存操作，零数据库 I/O、零外部网络请求。

### 上报频率与数据量

| 上报类型 | 频率 | 单次数据量 | 触发方式 |
|----------|------|------------|----------|
| CPS Report | 每 2 秒 | ~8 个数值字段 | 定时 |
| Move Report | 每 2 秒 | ~4 条采样 × 12 字段 ≈ 48 个值 | 定时 |
| Aim Report | 每次攻击（0.15s 冷却） | ~8 个字段 | 事件驱动 |
| Client Ready | 进服 1 次 | 几个字段 | 一次性 |
...
每秒 PyRpc 事件总量：
  CPS:   100 人 ÷ 2s  =  50 次/s
  Move:  100 人 ÷ 2s  =  50 次/s
  Aim:   30 人战斗 × 最高 6.6 次/s（0.15s 冷却） ≈ 200 次/s（极端情况）
  ─────────────
  总计：约 100 ~ 300 次/s
```
...
```java
dataManager.updateMove(player.getName(), data);
// → ConcurrentHashMap.computeIfAbsent() → 取 ClientData → ~12 个字段 get + 赋值
```

- **纯内存操作**，不碰磁盘、不碰数据库、不调外部 API
- 单次回调耗时约 **1~5 微秒**
- 300 次/s × 5μs = **1.5 毫秒/秒**，占主线程一个 tick（50ms）的不到 3%
- 内存开销：每个 `ClientData` 对象约 200~300 字节，100 人在线 ≈ **30KB**

### 为什么不会卡服务器

1. **无数据库 I/O** — 所有数据存内存 `ConcurrentHashMap`，玩家退出时自动清理
2. **无外部请求** — 不发 Webhook、不调外部 API（与 FapCpsLogger 的飞书推送不同）
3. **无定时轮询** — 服务端不主动轮询，完全被动接收客户端推送
4. **Move Report 聚合后取最新一条** — 即使缓冲区有 4 条采样，服务端遍历取最后一条后丢弃，不做复杂计算
5. **Aim 冷却封顶** — 0.15s 冷却限制了攻击事件的最高上报频率，避免疯狂点击刷屏

### 动态调参（无需重启）

所有采样频率和模块开关可通过服务端 `ConfigSync` 动态调整：

| 调整项 | 默认值 | 调整效果 |
|--------|--------|----------|
| `moveSampleInterval` | 0.5s | 调大 → 移动采样频率降低 |
| `moveReportInterval` | 2.0s | 调大 → 移动上报频率降低 |
| `cpsReportInterval` | 2.0s | 调大 → CPS 上报频率降低 |
| `enableMove` | true | 设为 false → 关闭移动模块（CPS/Aim 继续） |
| `enableCps` | true | 设为 false → 关闭 CPS 模块 |
| `enableAim` | true | 设为 false → 关闭准星模块 |

> **参考对比**：FapBtnBarBridge 的 NameTag 轮询是服务端每秒主动遍历所有在线玩家的 nsgb 缓存，开销更大但也不影响性能。FapAchBridge 是纯被动接收，开销更小。
...
| v0.0.2 | 2026-08-04 | moveCollector 新增 usingItem 状态采集（NoSlowDown 检测链路完善） |
| v0.0.2 (文档) | 2026-08-04 | README 新增性能影响分析章节 |
